...
times = pd.read_csv(base_directory + '/lris_p274_norm' + '/' + col + '_' + 'times' + '.csv')
...
if norm == 'poly':
    files = sorted(listdir(base_directory + 'lris_p274_norm' + '/' + norm)) # alphabetical order
elif norm == 'rolling':
    files = sorted(listdir(base_directory + 'lris_p274_norm' + '/' + norm + '/' + str(window)))
    
dat = []
for i in range(len(files)):
    if files[i][0] == col:
        if norm == 'poly':
            dat_i = data = pd.read_csv(base_directory + 'lris_p274_norm/' + norm + '/' + files[i])
        elif norm == 'rolling':
            dat_i = data = pd.read_csv(base_directory + 'lris_p274_norm/' + norm + '/' + str(window) + '/' + files[i])
        dat.append(dat_i)
...
# plt.imshow(flux_interp, extent = [np.min(grid), np.max(grid), np.min(phase_list), np.max(phase_list)], cmap = 'gray', aspect = 100, vmin= 0.5, vmax= 1.5)

# Plotting multiple periods 
